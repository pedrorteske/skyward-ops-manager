...
import { Quotation, Client, Flight } from '@/types/aviation';
import { generateQuotationPdfUrl, downloadQuotationPdf } from '@/lib/quotationPdfPreview';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const QuotationPdfPreview = ({ 
  ...
  useEffect(() => {
    if (open && quotation) {
      setIsLoading(true);
      generateQuotationPdfUrl({ quotation, client, flight })
        .then((url) => {
          setPdfUrl(url);
        })
        .finally(() => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Preview - Cotação {quotation.number}</DialogTitle>
        </DialogHeader>
