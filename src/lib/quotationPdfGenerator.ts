...
// Generates standardized PDFs for aviation service quotations
...
export function generateQuotationPDF({ quotation, client, flight }: QuotationPDFParams): void {
  ...
  // Save the PDF
  const fileName = `cotacao_${quotation.number.replace(/\//g, '-')}.pdf`;
  pdf.save(fileName);
}
