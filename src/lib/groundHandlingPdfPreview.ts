// Ground Handling PDF Preview Generator
...
function buildGroundHandlingPdf(data: GroundHandlingPDFData): jsPDF {
...
export async function generateGroundHandlingPdfUrl(data: GroundHandlingPDFData): Promise<string> {
  const pdf = buildGroundHandlingPdf(data);
  const blob = pdf.output('blob');
  return URL.createObjectURL(blob);
}

// Download PDF directly
export function downloadGroundHandlingPdf(data: GroundHandlingPDFData): void {
  const pdf = buildGroundHandlingPdf(data);
